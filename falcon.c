	...
	void *tmp, size_t tmp_len)
{
	...
	if (privkey_len < FALCON_PRIVKEY_SIZE(logn) || (pubkey != NULL && pubkey_len < FALCON_PUBKEY_SIZE(logn)) || tmp_len < FALCON_TMPSIZE_KEYGEN(logn))
	{
	...
	void *tmp, size_t tmp_len)
{
	...
	if (pubkey_len < FALCON_PUBKEY_SIZE(logn) || tmp_len < FALCON_TMPSIZE_MAKEPUB(logn))
	{
		return FALCON_ERR_SIZE;
	}
						  ...
						  void *tmp, size_t tmp_len)
{
	...
	if (expanded_key_len < FALCON_EXPANDEDKEY_SIZE(logn) || tmp_len < FALCON_TMPSIZE_EXPANDPRIV(logn))
	{
		return FALCON_ERR_SIZE;
	}
						 ...
						 void *tmp, size_t tmp_len)
{
	 ...
	 * Decode public key.
	 */
	if (Zf(modq_decode)(h, logn, pk + 1, pubkey_len - 1) != pubkey_len - 1)
	{
		return FALCON_ERR_FORMAT;
	}
	...
	if ((u + v) != sig_len)
	{
		 ...
		 * "padded" format.
		 */
		if ((sig_type == 0 && sig_len == FALCON_SIG_PADDED_SIZE(logn)) || sig_type == FALCON_SIG_PADDED)
		{
			while (u + v < sig_len)
			{
				if (es[u + v] != 0)
